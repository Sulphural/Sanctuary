...
// Extracted from AbilityPacketClientRequestStartAbilityHandler as a prototype for splitting its giant
// if-chain into per-category classes (PR #27 review). Logic is unchanged from before the move - only
// the shared internal statics it reaches back into (_resourceManager, IsOnCooldown, StartCooldown,
// SendFailure, IconTintId, DespawnNpc, BoomboxIdleAnimId, _castFxTagCounter) live on the old handler.
public sealed class BoomboxAbility : IConsumableAbility
{
    ...
    public bool Handle(GatewayConnection connection, AbilityPacketClientRequestStartAbility packet, int slot, ClientItem clientItem, ClientItemDefinition itemDefinition)
    {
        ...
        connection.Player.StartActionBarCooldown(2, slot, itemDefinition.Icon.Id, itemDefinition.NameId, clientItem.Count, BoomboxDurationMs, IconTintId(clientItem, itemDefinition));
