...
public sealed class DbHouseConfiguration : IEntityTypeConfiguration<DbHouse>
{
    public void Configure(EntityTypeBuilder<DbHouse> builder)
    {
        ...
        builder.Property(h => h.Id).IsRequired().ValueGeneratedNever();
        builder.HasIndex(h => new { h.CharacterId, h.Definition }).IsUnique();
        builder.HasOne(h => h.Character)
            .WithMany(c => c.Houses)
            .HasForeignKey(h => h.CharacterId);

        builder.Property(h => h.Definition).IsRequired();
        ...
        builder.Property(h => h.Created).IsRequired().HasDefaultValueSql("CURRENT_TIMESTAMP");
        builder.Property(h => h.LastVisited).IsRequired().HasDefaultValueSql("CURRENT_TIMESTAMP");
    }
}
