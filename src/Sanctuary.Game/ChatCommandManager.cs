...
public class ChatCommandManager : IChatCommandManager
{
    ...
    public bool Load()
    {
        // Every loaded Sanctuary assembly, not just this one: plenty of commands need Gateway-layer types
        // (the connection, packet handlers) that Sanctuary.Game can't reference, so they live in
        // Sanctuary.Gateway. Same discovery the packet handlers use.
        var assemblyTypes = AppDomain.CurrentDomain.GetAssemblies()
            .Where(x => x.FullName?.StartsWith(nameof(Sanctuary)) == true)
            .SelectMany(x => x.GetTypes());

        foreach (var interactionType in assemblyTypes)
        {
