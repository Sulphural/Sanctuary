        ...
        try
        {
            ...
            foreach (var entry in entries)
            {
                if (string.IsNullOrWhiteSpace(entry.Key) || entry.DropTable.Count == 0 ||
                    ...
                    // Same rule as a drop's Quantity, for the preview stand-in.
                    entry.PreviewQuantity <= 0 ||
                    // A non-positive quantity is granted as nothing at all: the player finishes whatever
                    // paid out and silently receives no item, so refuse it here rather than at grant time.
