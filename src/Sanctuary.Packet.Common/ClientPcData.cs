...
public class ClientPcData
{
    ...
    // state and picks between them on the character's member flag, so both must be set or
    // non-members get 0. This is the "/N" in the Adventurer's Journal quest counter, and the gate
    ...
    public byte[] Serialize()
    {
        ...
        writer.Write(0);                   // quests completed  -> +0x10FC (re-sent by op49/12)
        writer.Write(0);                   // tracked quest id  -> +0x10F4 (set later by op49 sub-opcodes)
